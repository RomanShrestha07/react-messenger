...
const MessageList = ({userData, fetchUsers}) => {
    ...
    const lastMessageRef = useCallback(node => {
        ...
        observer.current = new IntersectionObserver(entries => {
            if (entries[0].isIntersecting && max === false) { // get the last message element
    ...
    const NoContent = () => {
        return (
            <div style={{textAlign: 'center', paddingTop: 24}}>
                ...
                <Title level={5} style={{marginTop: 4}}>Jane Doe</Title>
        ...
        <div style={message_list_container}>
            {
                messages.map((message, index) => {
                    if (messages.length === index + 1) { // check if last element to set ref to last message
                        return (
                            <Message message={message} key={message.id} lastMessageRef={lastMessageRef}/>
                        )
                    } else {
                        if (index === 0) { // check if first element to set ref to fist message
                            return (
                                <Message message={message} key={message.id} firstMessageRef={firstMessageRef}/>
                            )
                        } else {
                            return (
